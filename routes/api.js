...
router.get('/maker/welcome1', async (req, res, next) => {
    ...
    if (!name ) return res.json({ status : false, creator : 'Ariel', message : "[!] masukan parameter name"})  
	if (!grup ) return res.json({ status : false, creator : 'Ariel', message : "[!] masukan parameter gpname"})  
    if (!member ) return res.json({ status : false, creator : 'Ariel', message : "[!] masukan parameter member"})  
	if (!pp ) return res.json({ status : false, creator : 'Ariel', message : "[!] masukan parameter pp"})  
    if (!bg ) return res.json({ status : false, creator : 'Ariel', message : "[!] masukan parameter bg"})  

	if ( !imgpp ) return res.json({ status : false, creator : 'Ariel', message : "[!] cek kembali url image pp"}) 
	if ( !bgimg ) return res.json({ status : false, creator : 'Ariel', message : "[!] cek kembali url image bg"}) 
	if(!apikey) return res.json(loghandler.notparam)
	if(listkey.includes(apikey)){
...
router.get('/maker/goodbye1', async (req, res, next) => {
	...
	if ( !imgpp ) return res.json({ status : false, creator : 'Ariel', message : "[!] masukan parameter pp Link pp dengan betul"}) 
	if ( !bgimg ) return res.json({ status : false, creator : 'Ariel', message : "[!] masukan parameter pp Link bg dengan betul"}) 
	if(!apikey) return res.json(loghandler.notparam)
	if(listkey.includes(apikey)){
