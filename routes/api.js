...
router.get('/attp', async(req, res, next) => {
  ...
  if(listkey.includes(apikey)) {
  let hasil = 'https://rest-privat-arielsp.herokuapp.com/api/maker/attp?text='+ text +'
...
router.get('/ttp', async(req, res, next) => {
  ...
  if(listkey.includes(apikey)) {
  let hasil = 'https://rest-privat-arielsp.herokuapp.com/api/ttp?text='+ text +'
  data = await fetch(hasil).then(v => v.buffer())
...
router.get('/emojimix', async(req, res, next) => {
  ...
  if(listkey.includes(apikey)) {
  let hasil = 'https://rest-privat-arielsp.herokuapp.com/api/maker/emojimix?emoji1=${emoji1}&emoji2=${emoji2}'
  data = await fetch(hasil).then(v => v.buffer())
  await fs.writeFileSync(__path +'/tmp/kont.png', data)
  res.sendFile(__path +'/tmp/kont.png')
  } else {
    res.json(loghandler.invalidKey)
  }
})
...
router.get('/download/fbariel', async(req, res, next) => {
  const apikey = req.query.apikey;
  const url = req.query.url;
  ...
  if(listkey.includes(apikey)){
    fetch(encodeURI(`https://rest-privat-arielsp.herokuapp.com/api/dowloader/fbdown?url=${url}`))
    .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/download/twitter', async(req, res, next) => {
  ...
  if(listkey.includes(apikey)){
    fetch(encodeURI(`https://rest-privat-arielsp.herokuapp.com/api/dowloader/twitter?url=${url}`))
    .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/download/tiktok', async(req, res, next) => {
  ...
  if(listkey.includes(apikey)){
    fetch(encodeURI(`https://rest-privat-arielsp.herokuapp.com/api/dowloader/tiktok?url=${url}`))
    .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/download/telesticker', async(req, res, next) => {
  ...
  if(listkey.includes(apikey)){
    fetch(encodeURI(`https://rest-privat-arielsp.herokuapp.com/api/dowloader/telesticker?url=${url}`))
    .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/arielganteng/welcome', async(req, res, next) => {
  const apikey = req.query.apikey;
  var name = req.query.name;
    ...
    if (!name ) return res.json({ status : false, creator : 'ArielSP', message : "[!] masukan parameter name"})  
	if (!grup ) return res.json({ status : false, creator : 'ArielSP', message : "[!] masukan parameter gpname"})  
    if (!member ) return res.json({ status : false, creator : 'ArielSP', message : "[!] masukan parameter member"})  
	if (!pp ) return res.json({ status : false, creator : 'ArielSP', message : "[!] masukan parameter pp"})  
    if (!bg ) return res.json({ status : false, creator : 'ArielSP', message : "[!] masukan parameter bg"})  

	if ( !imgpp ) return res.json({ status : false, creator : 'ArielSP', message : "[!] cek kembali url image pp"}) 
	if ( !bgimg ) return res.json({ status : false, creator : 'ArielSP', message : "[!] cek kembali url image bg"}) 
  if(!apikey) return res.json(loghandler.notparam)
  if(listkey.includes(apikey)){
  let hasil = `https://rest-privat-arielsp.herokuapp.com/api/maker/welcome1?name=${name}&gpname=${grup}&member=${member}&pp=${pp}&bg=${bg}`
  data = await fetch(hasil).then(v => v.buffer())
...
router.get('/arielganteng/goodbye', async(req, res, next) => {
  const apikey = req.query.apikey;
  var name = req.query.name;
    var grup = req.query.gpname;
    var member = req.query.member;
  ...
  if(!apikey) return res.json(loghandler.notparam)
  if(listkey.includes(apikey)){
  let hasil = `https://rest-privat-arielsp.herokuapp.com/api/maker/goodbye1?name=${name}&gpname=${grup}&member=${member}&pp=${pp}&bg=${bg}`
  data = await fetch(hasil).then(v => v.buffer())
         await fs.writeFileSync(__path +'/tmp/goodbye.jpg', data)
        res.sendFile(__path+'/tmp/goodbye.jpg')
  } else {
    res.json(loghandler.invalidKey)
  }
});
